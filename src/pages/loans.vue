...
const headers = [
  { title: 'Borrowed On', key: 'createdAt' },
  ...
  { title: 'Farmer ID NO.', key: 'farmer.idNumber' },
  { title: 'Farmer Code', key: 'farmer.farmerCode' },
  { title: 'Tranc Status', key: 'status' },
]

// Computed properties for loans data and pagination
const loans = computed(() => store.state.loans.loans)
const loading = computed(() => store.state.loans.loading)
const currentPage = computed(() => store.state.loans.currentPage)
const totalPages = computed(() => store.state.loans.totalPages)
const totalRecords = computed(() => store.state.loans.totalRecords)
const limit = computed(() => store.state.loans.limit)
...
watch(search, newValue => {
  ...
  debounceTimeout = setTimeout(() => {
    store.dispatch('loans/setSearchQuery', newValue) // Dispatch the search query after delay
...
watch(
  ...
  newOptions => {
    if (newOptions.page !== currentPage.value) {
      store.dispatch('loans/setPage', newOptions.page)
    }
  },
  { deep: true },
)
...
onMounted(() => {
  store.dispatch('loans/fetchLoans')
})
...
<template>
  <VCard>
    ...
    <VDataTable
      ...
      :headers="headers"
      :items="loans"
      loading-text="Loading loans... Please wait"
      :loading="loading"
    >
