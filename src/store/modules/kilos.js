// store/modules/kilos.js
import { getKilos } from '@/api/kiloService'
...
export const kilos = {
  ...
  actions: {
    async fetchKilos({ commit, state }) {
      ...
      try {
        const response = await getKilos(params)

        const data = response.data

        // Commit data to state
        commit('SET_KILOS', data.data.data)
        commit('SET_TOTAL_RECORDS', data.totalRecords)
        commit('SET_TOTAL_PAGES', data.totalPages)
      
      } catch (error) {
        console.error('Error fetching kilos:', error)
